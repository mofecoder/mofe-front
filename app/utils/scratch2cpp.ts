...
export class Sb3ToCppConverter {
  compiler!: string
  errorCommandSet: any
  loopCounterIndex: any
  nameOfAnswerVariable: any
  nonAsciiIdentifierSet: any
  unknownCommandSet: any
  usedListSet: any
  usedVariableSet: any
...
`
    ...
    for (const varID in variables) {
      const name = variables[varID][0]
      if (!this.usedVariableSet.has(name)) continue
      const initialValue = variables[varID][1]
      variableDeclareSource +=
        'Var ' +
        this.modifyVariableName(name) +
        '(' +
    ...
    for (const listID in lists) {
      const name = lists[listID][0]
      if (!this.usedListSet.has(name)) continue
      const initialValue = lists[listID][1]
      listDeclareSource += 'VarList ' + this.modifyListName(name) + '({'
      listDeclareSource += initialValue
        .map(
          (item: any) => 'Var(' + Sb3ToCppConverter.processLiteral(item) + ')'
        )
        .join(', ')
      listDeclareSource += '});\n'
    }
    ...
    if (this.unknownCommandSet.size > 0) {
      errorInfos.push({
        code: 1,
        message: Array.from(this.unknownCommandSet).join(',')
      })
    }
    /*
    ...
    if (this.errorCommandSet.size > 0) {
      errorInfos.push({
        code: 5,
        message: Array.from(this.errorCommandSet).join(',')
      })
    }
    return [cppSource, errorInfos]
  }

  modifyVariableName(name: any) {
    /*
    ...
    if (Sb3ToCppConverter.hasNonAscii(name))
      this.nonAsciiIdentifierSet.add(name)
    const escapeNonAscii = this.compiler === 'GCC'
    return (
      'var_' + Sb3ToCppConverter.escapeInvalidCharacter(name, escapeNonAscii)
    )
  }

  modifyListName(name: any) {
    this.usedListSet.add(name)
    if (Sb3ToCppConverter.hasNonAscii(name))
      this.nonAsciiIdentifierSet.add(name)
    const escapeNonAscii = this.compiler === 'GCC'
    return (
      'list_' + Sb3ToCppConverter.escapeInvalidCharacter(name, escapeNonAscii)
    )
  }

  modifyFunctionName(name: any) {
    if (Sb3ToCppConverter.hasNonAscii(name))
      this.nonAsciiIdentifierSet.add(name)
    const escapeNonAscii = this.compiler === 'GCC'
    return (
      'func_' + Sb3ToCppConverter.escapeInvalidCharacter(name, escapeNonAscii)
    )
  }

  modifyArgumentName(name: any) {
    if (Sb3ToCppConverter.hasNonAscii(name))
      this.nonAsciiIdentifierSet.add(name)
    const escapeNonAscii = this.compiler === 'GCC'
    return (
      'arg_' + Sb3ToCppConverter.escapeInvalidCharacter(name, escapeNonAscii)
    )
  }

  convertFrom(blockID: any, allBlocksInfo: any) {
    /*
        Args:
            blockID (str)
    ...
    if (blockID) {
      try {
        ...
        switch (opcode) {
          ...
          case 'control_forever':
            innerSnippet = this.processValueInfo(
              blockInfo.inputs.SUBSTACK,
              allBlocksInfo
            )
            snippet =
          ...
          case 'control_repeat': {
            ...
            const cntName = `cnt_${this.loopCounterIndex}`
            this.loopCounterIndex++
            snippet =
              `for (int ${cntName}=0;${cntName}<${this.processValueInfo(
                blockInfo.inputs.TIMES,
                allBlocksInfo
              )}.asNumber();${cntName}++){\n` +
              Sb3ToCppConverter.indent(innerSnippet, 4) +
              '}\n'
          ...
          case 'control_repeat_until':
            ...
            snippet =
              `while (!${this.processValueInfo(
                blockInfo.inputs.CONDITION,
                allBlocksInfo
              )}){\n` +
              Sb3ToCppConverter.indent(innerSnippet, 4) +
              '}\n'
          ...
          case 'looks_sayforsecs':
            value = this.processValueInfo(
              blockInfo.inputs.MESSAGE,
              allBlocksInfo
            )
          ...
          case 'operator_mathop': {
            value = this.processValueInfo(blockInfo.inputs.NUM, allBlocksInfo)
            let opName: string = blockInfo.fields.OPERATOR[0]
            if (opName === '10 ^') {
              ...
              if (opName in dic) {
                opName = dic[opName as keyof typeof dic]
              }
          ...
          case 'operator_or':
            value = this.processValueInfo(
              blockInfo.inputs.OPERAND1,
              allBlocksInfo
            )
            value2 = this.processValueInfo(
              blockInfo.inputs.OPERAND2,
              allBlocksInfo
            )
            snippet = `(${value} || ${value2})`
            break
          ...
          case 'operator_join':
            value = this.processValueInfo(
              blockInfo.inputs.STRING1,
              allBlocksInfo
            )
            value2 = this.processValueInfo(
              blockInfo.inputs.STRING2,
              allBlocksInfo
            )
          ...
          case 'operator_letter_of':
            value = this.processValueInfo(
              blockInfo.inputs.LETTER,
              allBlocksInfo
            )
            value2 = this.processValueInfo(
              blockInfo.inputs.STRING,
              allBlocksInfo
            )
          ...
          case 'procedures_call': {
            funcName = this.modifyFunctionName(
              blockInfo.mutation.proccode.split(' ')[0]
            )
            value = []
            const argIDs = JSON.parse(blockInfo.mutation.argumentids)
            for (let i = 0; i < argIDs.length; i++) {
              value.push(
                this.processValueInfo(
                  blockInfo.inputs[argIDs[i]],
                  allBlocksInfo
                )
              )
            }
          ...
          case 'procedures_definition':
            ...
            funcName = this.modifyFunctionName(
              allBlocksInfo[procProtoBlockID].mutation.proccode.split(' ')[0]
            )
            argNames = JSON.parse(
              allBlocksInfo[procProtoBlockID].mutation.argumentnames
            )
      ...
      // 「次の処理」が存在する場合
      const nextBlockID = allBlocksInfo[blockID].next
      if (nextBlockID) {
  ...
  processValueInfo(valueInfo: any, allBlocksInfo: any) {
    if (!valueInfo) return ''
    switch (valueInfo[0]) {
      case 3: // other block, or variable
      case 2: // CONDITION だとこのパターンもある？
        if (typeof valueInfo[1] === 'string') {
          // other block
  ...
  static processLiteral(obj: any) {
    ...
    if (typeof obj === 'string') {
      /*
            ...
            */
      if (isNumeric(obj) && '' + Number(obj) === obj) {
        // 標準的な記法かのチェック
  ...
  static indent(snippet: any, numSpace: any) {
    /*
        Args:
    ...
    const spaces = Array(numSpace + 1).join(' ')
    return lines.map((line: any) => spaces + line + '\n').join('')
  }

  static hasNonAscii(name: any) {
    /* returns whether the string contains non-ascii characters */
    return !name.match(/^[\x20-\x7E]*$/)
  }

  static escapeInvalidCharacter(name: any, escapeNonAscii = false) {
    /*
  ...
  static makeByteLengthNice(src: any) {
    /*
    ...
    const crlfPattern = '\\n'
    src = src
      .replace(new RegExp(lfPattern, 'g'), '\n')
      .replace(new RegExp(crlfPattern, 'g'), '\r\n')
